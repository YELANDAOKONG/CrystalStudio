...
- 理事会不执行工具。选中的一份方案里的全部 `tool_calls` 原样返回（可一次多条，便于并行读取），由调用方（任意兼容的 CrystalCode）按自己的批准策略执行。
...
- 旁路检出 [CrystalCode](https://github.com/YELANDAOKONG/CrystalCode)，路径为 `../CrystalCode`
- 各席位所用提供商的 API Key（解析顺序与 CrystalCode 相同）

## 教程

### 1. 把三个仓库放在一起

在同一父目录下并列克隆 Crystal、CrystalCode 和本仓库：
...
### 2. 把 API Key 放到 CrystalCode 能读到的地方
...
`~/.crystal/config.json` 的读法与 CrystalCode 相同。
...
在 [CrystalCode](https://github.com/YELANDAOKONG/CrystalCode)
...
CrystalCode 本地报 `cannot replay reasoning blocks on Chat Completions`，
...
CrystalCode 目录里已经存在的提供商和模型。支持思考的模型可再写
...
| `--harness-home <path>` | 共享的 CrystalCode 主目录（默认 `CRYSTAL_HOME`，否则 `~/.crystal`） |
...
与 [CrystalCode](https://github.com/YELANDAOKONG/CrystalCode)
共享的提供商和密钥：
...
CrystalCode 目录可用 `CRYSTAL_HOME` 或 `--harness-home` 覆盖。
